...
%%               [{report,{eunit_jenkins_report,[{dir,"."}]}}]).'''

-module(eunit_jenkins_report).
...
write_report_to(TestSuite, FileDescriptor) ->
    ...
    write_testcases(TestSuite, lists:reverse(TestSuite#testsuite.testcases), FileDescriptor),
...
%% ----------------------------------------------------------------------------
write_testcases(_TestSuite, [], _FileDescriptor) -> void;
write_testcases(TestSuite, [TestCase| Tail], FileDescriptor) ->
    write_testcase(TestSuite, TestCase, FileDescriptor),
    write_testcases(TestSuite, Tail, FileDescriptor).
...
write_testcase(
        ...
        #testcase{
            ...
            output = Output},
        FileDescriptor) ->
    ModuleName = escape_suitename(SuiteName),
    Module = list_to_atom(ModuleName),
    Source = proplists:get_value(source, Module:module_info(compile)),
    AppName = case re:run(Source,".*\/([a-z0-9_]+)(-[0-9\.]+)?\/(src|tests)\/" ++ ModuleName ++ ".erl", [global,{capture,[1],list}]) of
        {match,[App]} ->
            App ++ ".";
        _ ->
            io:format("Couldn't match source pattern for module ~s (source: ~s). Please check *.app file.~n", [ModuleName, Source]),
            "unknown."
    end,
    ...
    NameTokens = string:tokens(escape_attr(Name), " "),
